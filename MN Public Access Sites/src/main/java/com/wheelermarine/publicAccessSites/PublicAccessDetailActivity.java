...
public class PublicAccessDetailActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        TextView name = getWindow().findViewById(R.id.publicAccessName);
        name.setText(access.getName());

        TextView lake = getWindow().findViewById(R.id.publicAccessLake);
        lake.setText(access.getLake());

        TextView county = getWindow().findViewById(R.id.publicAccessCounty);
        county.setText(access.getCounty());

        TextView launch = getWindow().findViewById(R.id.publicAccessLaunch);
        launch.setText((access.getRamps() > 0 ? access.getRamps() + " " : "") + (!access.getRamp().isEmpty() ? access.getRamp() + " " : "") + access.getLaunch());

        TextView docks = getWindow().findViewById(R.id.publicAccessDocks);
        docks.setText(access.getDocks() > 0 ? "Yes (" + access.getDocks() + ")" : "No");

        TextView directions = getWindow().findViewById(R.id.publicAccessDirections);
        ...
        if (status != ConnectionResult.SUCCESS) {
            ...
            if (!mapTypeStr.matches("[0-9]+"))
                mapTypeStr = String.valueOf(GoogleMap.MAP_TYPE_NORMAL);
        ...
        if (locationManager != null && checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            ...
            if (location != null) {
                TextView distance = getWindow().findViewById(R.id.publicAccessDistance);

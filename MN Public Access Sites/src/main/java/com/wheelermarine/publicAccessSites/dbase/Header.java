...
public class Header {
	...
	public static int readLittleEndian(DataInput in, int bytes) throws IOException {
		...
		if (bytes <= 0) throw new IllegalArgumentException("Number of bytes must be a positive integer.");
		if (bytes > 4) throw new IllegalArgumentException("Number of bytes cannot be greater than 4.");
		int bigEndian = 0;

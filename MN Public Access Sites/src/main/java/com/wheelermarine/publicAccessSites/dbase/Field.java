...
public class Field {
    ...
    public Field(DataInput in) throws IOException {
        ...
        fieldName = new String(nameBytes, StandardCharsets.US_ASCII).trim();

        byte[] typeBytes = new byte[1];
        in.readFully(typeBytes);
        type = FieldType.getType(new String(typeBytes, StandardCharsets.US_ASCII));
